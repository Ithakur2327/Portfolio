...
/* ─── Live clock — "04:19 PM IST" format ─────────────── */
function LiveClock() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      /* diff text */
      const viewerOff = -now.getTimezoneOffset(); // minutes
      const istOff    = 330;                       // IST = UTC+5:30
      const diff      = istOff - viewerOff;
      if (diff === 0) { setDiff("same time"); return; }
      const h = Math.floor(Math.abs(diff) / 60);
      const m = Math.abs(diff) % 60;
      setDiff(`${h}${m ? `:${String(m).padStart(2,"0")}` : ""}h ${diff > 0 ? "ahead" : "behind"}`);
    };
    update();
  ...
  return (
    <span style={{ fontFamily: "'Geist Mono', monospace" }}>
      {time || "-- : -- AM IST"}{" "}
      {diff && <span style={{ color: "#52525b" }}>// {diff}</span>}
    </span>
  );
}
...
function Row({ icon, href, newTab, children }: {
  ...
  if (href) return (
    <a href={href} target={newTab ? "_blank" : undefined} rel="noreferrer" style={base}
      onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = "#e4e4e7"}
      onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = "#a1a1aa"}
    >{icon}<span>{children}</span></a>
  );
  return <div style={{ ...base, cursor: "default" }}>{icon}<span>{children}</span></div>;
}
...
function SocialTile({ href, label, icon, iconBg, iconBorder, iconColor, last }: {
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" style={{
      ...
      padding: "14px 16px", borderRight: last ? "none" : B,
  ...
  return (
    <>
      <style>{`
        ...
        .hero-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 40px; }
        ...
        @media (max-width: 540px) {
          ...
          .hero-av-box { padding: 10px 8px !important; }
          .hero-av, .hero-av-lit { width: 80px !important; height: 80px !important; font-size: 18px !important; }
        }
        @media (min-width: 541px) and (max-width: 768px) {
          .hero-av, .hero-av-lit { width: 100px !important; height: 100px !important; font-size: 22px !important; }
        }

        /* avatar lights */
        .av-lit-wrap .hero-av     { transition: opacity 0.3s, box-shadow 0.3s; }
        .av-lit-wrap .hero-av-lit { position:absolute; inset:0; border-radius:50%; opacity:0; transition:opacity 0.3s; }
        .av-lit-wrap.lit .hero-av     { opacity: 0; }
        .av-lit-wrap.lit .hero-av-lit { opacity: 1; }
      `}</style>

      <section id="about" style={{
        marginTop: 52, background: BG, borderBottom: B,
        ...
        {/* ── PROFILE HEADER ── */}
        <div className="hero-top" style={{ borderBottom: B }}>
          ...
          <div className="hero-av-box" style={{
            ...
            >
              ...
              <div className="hero-av" style={{
                ...
                fontFamily: "'Geist Mono',monospace",
                boxShadow: lit ? "0 0 40px rgba(251,191,36,0.3)" : "none",
              }}>IT</div>
              {/* lit */}
              <div className="hero-av-lit" style={{
                width: 120, height: 120, borderRadius: "50%",
              ...
              <span className="pulse" style={{
                ...
                background: "#22c55e", border: "2.5px solid #09090b", display: "block",
              }} />
            </div>
          </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
            {/* spacer — no code hint text */}
            <div style={{ flex: 1 }} />

            {/* Name */}
            <div style={{ borderTop: B, padding: "10px 18px 6px" }}>
              <h1 style={{
            ...
            {/* Flip */}
            <div style={{
              borderTop: B, padding: "9px 18px 10px",
        ...
        {/* ── INFO BLOCK — no hatch, dot background shows through ── */}
        <div style={{ padding: "16px 18px 14px" }}>
          ...
          <div className="hero-grid">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
              ...
              <Row href="https://indreshthakur.dev" newTab
              ...
              >indreshthakur.dev</Row>

              <Row href="/resume.pdf" newTab
                icon={<IBox><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="15" y2="17"/></svg></IBox>}
              >Resume</Row>
            </div>
        ...
        <div className="hero-social" style={{ display: "flex", borderTop: B }}>
          ...
          />
          <SocialTile href="https://linkedin.com/in/indresh-thakur" label="LinkedIn"
